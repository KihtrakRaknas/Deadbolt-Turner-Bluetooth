    ...
    while True:
        ...
        if rssi is None:
            ...
            if old_rssi is None:
                ...
                print(str(all(-20 <= el <= 0 for el in (old_rssis[-5:]))) + " & " + str(not any(-20 <= el <= 0 for el in (old_rssis[:75]))))
                if all(-20 <= el <= 0 for el in (old_rssis[-5:])) and not any(-20 <= el <= 0 for el in (old_rssis[:75])):
                    asyncio.create_task(open_door())
